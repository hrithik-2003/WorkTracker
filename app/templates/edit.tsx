import { View, Text, TextInput, ScrollView, TouchableOpacity, Modal, FlatList, Alert, ActivityIndicator } from "react-native";
...
export default function EditTemplateScreen() {
    ...
    const loadData = async () => {
        setLoading(true);
        const [exercisesData, templateData] = await Promise.all([
            fetchExercises(),
            id ? getTemplateDetails(id) : null
        ]);

        // Map exercises
        const mapped: Exercise[] = exercisesData.map((ex: any) => ({
            id: ex.id,
            name: ex.name,
        ...
        if (templateData) {
            ...
            setAddedExercises(
                templateData.exercises.map(ex => ({
                    id: ex.id,
                    name: ex.name,
                    sets: String(ex.recomendedSets || 3),
                }))
            );
        }
        setLoading(false);
    ...
    const handleSave = async () => {
        ...
        const result = await updateTemplate(id, name, description, exercisesPayload);
        setSaving(false);
    ...
    return (
        <ScreenLayout>
            ...
            <View className="flex-row justify-between items-center mb-6 mt-4">
                <Text className="text-white text-2xl font-bold">Edit Routine</Text>
                <Button
